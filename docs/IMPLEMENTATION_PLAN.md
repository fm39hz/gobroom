...
| M8 | Passive health, limits, performance and usage | Partial | Typed classified outcomes, scoped passive route/connection/provider evidence, reset-aware cooldowns, bounded feedback ranking, completion TTFT/throughput, request-class EWMA selection, half-open single trials and opt-in quota polling now run in the kernel path. Opportunistic quota enrichment is now singleflight/cooldowned after quota evidence. Remaining: durable evidence projections and session affinity. See [Passive health](PASSIVE_HEALTH_ROUTING.md). |
