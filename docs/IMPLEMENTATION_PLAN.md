...
| M10 | Physical identity, capability and modality policies | Partial; next kernel foundation | Typed support states, request-requirement compilation, profile persistence, typed TokenLimits, Physical identity/revision fields, source fidelity/evidence persistence and pure declared/guaranteed/available projection now exist. Kernel eligibility checks input/output/total budget when limits are declared. Domain presentation no longer exposes boolean capability maps. Expose projections through runtime/control surfaces, then finish route-effective evidence and remaining profile semantics. Never silently strip or downgrade user intent. See [Physical models](PHYSICAL_MODELS.md). |
