...
| M7 | Upstream credential lifecycle and OAuth | Partial; generic lifecycle done | Static API-key/Bearer resolution, typed OAuth token parsing, refresh-token flow and kernel refresh-once after 401/403 are implemented. Bind provider-specific OAuth configs/flows, persist refreshed token state and add refresh-race fixtures. This is distinct from client auth to hosted GoBroom. |
