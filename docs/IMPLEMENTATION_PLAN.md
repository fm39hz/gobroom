...
| M8 | Passive health, limits, performance and usage | Partial | Typed classified outcomes, passive route evidence, reset-aware cooldowns, bounded feedback ranking, completion TTFT/throughput and opt-in quota polling now run in the kernel path. Remaining: request-class aggregation, scoped breakers/half-open state, opportunistic quota enrichment and durable evidence projections. See [Passive health](PASSIVE_HEALTH_ROUTING.md). |
