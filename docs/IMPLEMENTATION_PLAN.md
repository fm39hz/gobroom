...
| M10 | Physical identity, capability and modality policies | Partial; next kernel foundation | Typed support states and request-requirement compilation now exist in the kernel, with conservative hard eligibility for typed profiles. Persist route/Physical profiles, replace boolean catalog capabilities, add identity/revision and source fidelity, token limits, evidence, route-effective profiles and declared/guaranteed/available projections. Never silently strip or downgrade user intent. See [Physical models](PHYSICAL_MODELS.md). |
