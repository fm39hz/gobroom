...
| M10 | Physical identity, capability and modality policies | Partial; next kernel foundation | Typed support states, request-requirement compilation, profile persistence, typed TokenLimits, Physical identity/revision fields, source fidelity/evidence persistence, pure declared/guaranteed/available projection and Physical IPC/TUI projection display now exist. Kernel eligibility checks input/output/total budget when limits are declared. Domain presentation no longer exposes boolean capability maps. Finish route-effective evidence and remaining profile semantics. Never silently strip or downgrade user intent. See [Physical models](PHYSICAL_MODELS.md). |
