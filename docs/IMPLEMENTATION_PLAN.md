...
| M10 | Physical identity, capability and modality policies | Partial; next kernel foundation | Typed support states, request-requirement compilation, profile persistence and typed TokenLimits through catalog/discovered/Physical/snapshot now exist. Kernel eligibility checks input/output/total budget when limits are declared. Domain presentation no longer exposes boolean capability maps. Add identity/revision and source fidelity, evidence, route-effective profiles and declared/guaranteed/available projections. Never silently strip or downgrade user intent. See [Physical models](PHYSICAL_MODELS.md). |
