...
| M5 | Canonical response-event contract | Partial; lifecycle foundation done | Kernel now defines canonical start/text/thinking/tool/usage/completed/error events and stream hooks; Anthropic text/tool conversion emits them. Finish Responses/OpenAI event extraction, content-block semantics, malformed-stream fixtures and lossless renderer boundaries before broadening M6. |
