...
| M8 | Passive health, limits, performance and usage | Partial | Typed classified outcomes, scoped passive route/connection/provider evidence, reset-aware cooldowns, bounded feedback ranking, completion TTFT/throughput and opt-in quota polling now run in the kernel path. Remaining: request-class aggregation, half-open trials, opportunistic quota enrichment and durable evidence projections. See [Passive health](PASSIVE_HEALTH_ROUTING.md). |
