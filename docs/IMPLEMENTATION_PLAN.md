...
| M8 | Passive health, limits, performance and usage | Partial; runtime core done | Typed outcomes, scoped route/connection/provider evidence, reset-aware cooldowns, bounded feedback ranking, TTFT/throughput, request-class EWMA, half-open trials, session affinity, durable health/usage IPC/TUI, usage summaries, route-order explanation, opt-in quota polling and opportunistic enrichment are implemented. Remaining: retention policy and log streaming. See [Passive health](PASSIVE_HEALTH_ROUTING.md). |
