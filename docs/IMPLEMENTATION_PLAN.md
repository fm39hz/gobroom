...
| M5 | Canonical response-event contract | Partial; semantic stream foundation done | Kernel defines canonical start/text/thinking/tool/usage/completed/error events and stream hooks. Anthropic and OpenAI Chat/Responses SSE observers emit semantic events while preserving upstream output; malformed JSON and Responses continuity metadata are covered. Finish richer content-block semantics and lossless renderer boundaries. |
| M6 | Responses/Anthropic semantic compatibility | Partial; reasoning foundation started | Normalized reasoning now distinguishes absent `inherit` from explicit modes; layered precedence is defined; Anthropic uses real `thinking`/`output_config.effort`, and cross-protocol OpenAI Responses uses native `reasoning`. Finish Gemini dialect wiring, persisted presets/combo defaults, continuity/output-item fixtures and full content-block/tool/thinking usage parity. |
