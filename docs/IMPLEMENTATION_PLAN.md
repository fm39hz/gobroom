...
| M6 | Responses/Anthropic semantic compatibility | Partial; reasoning foundation started | Normalized reasoning now distinguishes absent `inherit` from explicit modes, and Anthropic cross-protocol requests use real `thinking`/`output_config.effort` fields. Finish OpenAI/Gemini dialect translators, presets/precedence, continuity/output-item fixtures and full content-block/tool/thinking usage parity. |
