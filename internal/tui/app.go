...
func (m *app) toggleExposure() tea.Cmd {
	...
	case physicalModel:
		...
		return m.invoke("physical_models.upsert", map[string]any{"name": value.Name, "sources": value.Sources, "policy": value.Policy, "capabilities": value.Capabilities, "discoverable": value.Discoverable, "enabled": value.Enabled})
	case comboModel:
		value.Discoverable = !value.Discoverable
		m.loading = true
