...
func fetchStatus(path string) tea.Cmd {
	return func() tea.Msg {
		response, err := daemon.CallIPC(context.Background(), path, daemon.IPCRequest{ID: "tui-status", Method: "status"})
		if err != nil {
			return statusMsg{err: err}
		}
		data, err := json.MarshalIndent(response.Result, "", "  ")
		return statusMsg{value: string(data), err: err}
	}
}
...
func (m model) Update(message tea.Msg) (tea.Model, tea.Cmd) {
	...
	case statusMsg:
		m.status, m.err = msg.value, msg.err
		return m, nil
	case tea.KeyPressMsg:
		...
		case "q", "ctrl+c":
			return m, tea.Quit
		case "r":
			return m, fetchStatus(m.ipcPath)
		}
	}
	return m, nil
}
